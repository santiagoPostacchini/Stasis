...
{
    [Header("Detección de plataforma (RB)")]
    ...
    private void LateUpdate()
    {
        ...
        if (hitGround)
        {
            // ¿Tiene el componente en el árbol?
        ...
        // 3) Coyote time: si no hubo hitGround este frame pero estábamos en plataforma, aguantamos unos frames
        if (hitGround)
        {
            coyote = coyoteFrames;
        }
        ...
        {
            coyote--;
            hitGround = true; // mantenemos consideración de contacto durante la gracia
        }
