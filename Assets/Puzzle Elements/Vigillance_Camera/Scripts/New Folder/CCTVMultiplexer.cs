...
public class CCTVMultiplexer : MonoBehaviour
{
    ...
    [Header("URP")]
    [Tooltip("Índice del Renderer limpio (sin features) en el URP Asset")]
    public int urpRendererIndex = -1;

    // === OVERLAY LAYER (se dibuja encima) ===
    [Header("Overlay (Top Layer)")]
    [Tooltip("Cualquier objeto en esta Layer se dibuja por ENCIMA de todo lo demás.")]
    public LayerMask overlayTopLayer = 0; // elegí la layer a superponer
    ...
    void LateUpdate()
    {
        ...
        for (int i = 0; i < _cams.Count; i++)
        {
            ...
            // Pose/rotación (horizonte nivelado si así está configurado)
            Transform p = v.Pivot;
            ...
            if (layerMaskBase == 0) layerMaskBase = ~0; // fallback si quedó vacío
