...
{
    public class StasisGun2 : MonoBehaviour
    {
        [Header("Visual")]
        ...
        [Header("Raycast")]
        [Tooltip("Radio del SphereCast para perdonar errores de punter�a.")]
        [SerializeField] private float radiusStasis = 0.2f;
        [Tooltip("Distancia m�xima del raycast/spherecast.")]
        ...
        private void Start()
        {
            ...
            // C�mara: prioriza la de hijo/padre, luego Camera.main
            ...
            if (!mainCam && debugLogs)
                Debug.LogWarning("[StasisGun] No se encontr� c�mara. Asigna una Camera en escena.");
        }
        ...
        private void TryApplyStasis()
        {
            ...
            // 1) Ray desde el centro de pantalla, con un peque�o offset del near clip
            Ray ray = GetCenterScreenRay(mainCam);

            // 2) Hiteo: primero SphereCast para perdonar, si no, Raycast fino
            bool gotHit = Physics.SphereCast(ray, radiusStasis, out RaycastHit hit, maxDistance, layer, QueryTriggerInteraction.Collide)
        ...
        {
            // Peque�o delay para sincronizar con el FX
        ...
        {
            // Un frame / peque�o delay para que no compita con otros efectos
            yield return null;
            ...
            if (gotHit)
            {
                ...
                if (stasisOrigin)
                {
                    ...
                    if (debugLogs && aimVsBeamAngle > 5f)
                        Debug.LogWarning($"[StasisGun] Aim vs Beam angle = {aimVsBeamAngle:F1}� (posible desalineaci�n de FX o mano).");
                }
            }
            else
            {
